...
export const addToCart = createAsyncThunk(
  ...
  async (body, thunkAPI) => {
    try {
      const response = await axiosInstance.post("/users/cart", body); // body는  productId: product._id
      return response.data;
    } catch (error) {
      console.log(error);
      return thunkAPI.rejectWithValue(error.response.data || error.message);
...
export const getCartItems = createAsyncThunk(
  "user/getCartItems",
  async ({ cartItems, userCart }, thunkAPI) => {
    try {
      const response = await axiosInstance.get(
        `/products/${cartItems}?type=array`
      );
    ...
    try {
      ...
      response.data.cart.forEach((cartItem) => {
        response.data.productInfo.forEach((productDetail, index) => {
          if (cartItem._id === productDetail.id) {
